		...
		def assistant(self,query,w=None):
			...
			if w:
				for i in a_data["user"]:
					if query.lower() in a_data["user"][i]:
						...
						if i == "geliştirici":
							...
							self.return_a_w_msg(qery, a_data["assistant"][i].format(haftaGün[time.strftime("%w")]))
						elif i == "hava":
							...
							for d in reminders:
								...
								if date(int(y),int(a),int(g)).isocalendar()[1] == datetime.now().isocalendar()[1]:
									#self.return_a_w_msg3(haftaGün[str(date(int(y),int(a),int(g)).isocalendar()[2])]+" günü "+str(len(reminders[d]))+" tane hatırlatıcı bulunuyor.")
									for t in reminders[d]:
										self.return_a_w_msg3(haftaGün[str(date(int(y),int(a),int(g)).isocalendar()[2])]+", "+t+", "+reminders[d][t])
								else:
									#self.return_a_w_msg3(d+" tarihinde "+str(len(reminders[d]))+" tane hatırlatıcı bulunuyor.")
									for t in reminders[d]:
				...
				if query == "none":
					...
					for i in a_data["user"]:
						if query in a_data["user"][i]:
							...
							if i == "geliştirici":
								...
								self.return_a(a_data["assistant"][i].format(time.strftime("%d"),yılAy[time.strftime("%m")],haftaGün[time.strftime("%w")]))
							elif i == "durum":
								self.return_a(choice(a_data["assistant"][i]))
							elif i == "durumD":
								pass
								...
								il,ilce,havad = hava.havaD()
								self.return_a(query, a_data["assistant"][i].format(il+" "+ilce if ilce else il,havad))
							elif i == "havaY":
								il,ilce,havad = hava.havaDY()
								self.return_a(query, a_data["assistant"][i].format(il+" "+ilce if ilce else il,havad))
							elif i == "doviz":
								self.return_a(query, doviz.doviz())
							elif i == "google_ac":
					...
					for l in a_data["user"]["lamba_ac"]:
						if l in query.lower():
							...
							if lamp2:
								...
								print(lamp2)
								print(lampp2)
								self.return_a(query, lamp+" ve "+lamp2+" ışığı açılıyor")
							else:
								print(lamp)
								print(lampp)
								self.return_a(query, lamp+"ışığı açılıyor")
					...
					for l in a_data["user"]["lamba_kapat"]:
						if l in query.lower():
							...
							if lamp2:
								...
								print(lamp2)
								print(lampp2)
								self.return_a(query, lamp+" ve "+lamp2+" ışığı kapatılıyor")
							else:
								print(lamp)
								print(lampp)
								self.return_a(query, lamp+"ışığı kapatılıyor")
					...
					if on:
						self.return_a(query+"İle İlgili Google Araması")
						web.search_google(query)
